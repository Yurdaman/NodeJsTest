...
describe("Cross-Page Tests", function () {
  ...
  it("referrer should be filled when navigating from Hood River", async function () {
    try {
      ...
      await page.goto("http://localhost:3000/tours/hood-river", { waitUntil: 'domcontentloaded' });
  ...
  it("referrer should be filled when navigating from Oregon Coast", async function () {
    try {
      await page.goto("http://localhost:3000/tours/oregon-coast", { waitUntil: 'domcontentloaded' });
      ...
      console.log('Referrer value:', referrerValue);
      expect(referrerValue).to.equal("http://localhost:3000/tours/oregon-coast");
    } catch (error) {
      console.error('Test failed:', error);
      throw error;
    }
  ...
  it("referrer should be empty when going directly", async function () {
    try {
      await page.goto("http://localhost:3000/tours/request-group-rate", { waitUntil: 'domcontentloaded' });
